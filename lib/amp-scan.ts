...
function createBroadcastQueryPacket() {
  ...
  return Buffer.concat([networkData, frame]);
}

function parseBasicInfoPacket(msg: Buffer, rinfo: RemoteInfo) {
...
export async function broadcastDiscovery(): Promise<Array<{ ip: string; mac: string; name: string; version: string }>> {
  return new Promise((resolve) => {
    ...
    const basicInfoPacketHandler = (msg: Buffer, rinfo) => {
      ...
      devices.set(basicInfo.mac, basicInfo);
    };

    setTimeout(() => {
      networkAdapter.removeListener("message", basicInfoPacketHandler);
